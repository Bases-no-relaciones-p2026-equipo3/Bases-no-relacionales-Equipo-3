...
| **`infra/`** | Configuración de infraestructura. La subcarpeta `cassandra/` contiene los archivos `cassandra.yaml` y `cassandra-rackdc.properties` que definen los parámetros del nodo Cassandra. La subcarpeta `spark/` contiene los Dockerfiles que construyen las imágenes de Spark utilizadas para procesamiento.  y tambien esta la de neo4j|
